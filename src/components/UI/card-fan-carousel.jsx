...
const ARROW_CLASSES =
  "relative flex items-center justify-center rounded-full border-[1.5px] border-surface-border bg-surface/80 backdrop-blur-[16px] text-paper-muted cursor-pointer shrink-0 z-30 outline-none shadow-card hover:border-ember/40 hover:text-ember active:opacity-70 transition-colors duration-300";

export default function CardFanCarousel({ cards, renderCard, cardClassName = "" }) {
  ...
  return (
    <section className="flex flex-col items-center w-full mt-24 lg:mt-32 py-4 lg:py-8 px-4 md:px-8 relative z-20">
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem] min-h-[50vh]">
          {cards.map((card, index) => {
            ...
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className={`fan-card block cursor-pointer ${cardClassName}`}>
                {content}
              </a>
            ) : (
              <div key={index} className={`fan-card ${cardClassName}`}>
                {content}
              </div>
            );
          })}
      ...
      {needsPagination && (
        <div className="flex items-center justify-center gap-4 mt-4 md:mt-6 z-30">
          ...
          <div className="flex items-center gap-2">
            {cards.map((_, i) => (
              <span key={i} className={`w-2 h-2 rounded-full transition-all duration-300 ${i === centerIndex ? "bg-ember scale-[1.3]" : "bg-surface-border"}`} />
            ))}
          </div>
